...
const f5::json::assertion::checker f5::json::assertion::contains_checker = [](
    ...
    if( array.isarray() ) {
        for ( std::size_t index{}; index < array.size(); ++index ) {
            const auto valid = validation::first_error(an, an.spos / rule, an.dpos / index);
    ...
    if ( array.isarray() ) {
        if ( part.isarray() ) {
            ...
            for ( std::size_t index{}; index < std::min(psize, dsize); ++index ) {
                auto valid = validation::first_error(an, an.spos / rule / index, an.dpos / index);
            ...
            for ( std::size_t index{}; index < array.size(); ++index ) {
                auto valid = validation::first_error(an, an.spos / rule, an.dpos / index);
                if ( not valid ) return valid;
                an.merge(std::move(valid));
            }
        }
